...
public class SalaControllerHttp {
    ...
    public ResponseEntity<List<SalaResponse>> listar(HttpServletRequest request,
        ...
        // String autorization =
        // Optional.ofNullable(request.getHeader("Authorization")).orElseThrow(() -> new
        // ErroDeAutenticacaoGeral("Usuário não autenticado"));
        // this.capturarUsernameDoToken(autorization);
        List<SalaResponse> lista = this.salaManager.listar(usernameDono, nome, categoria).stream()
                .map(SalaResponse::new).toList();
        return ResponseEntity.ok(lista);
    }

    @GetMapping("/_convidado") // lista as salas que o usuário é convidado
    public ResponseEntity<List<SalaResponse>> listarPorParticipante(HttpServletRequest request,
            @RequestParam(defaultValue = "") String usernameParticipante) {
        List<SalaResponse> lista = this.salaManager.listarPorParticipante(usernameParticipante).stream()
                .map(SalaResponse::new).toList();
        return ResponseEntity.ok(lista);
    }

    @GetMapping("/_dono") // lista as salas que o usuário é dono
    public ResponseEntity<List<SalaResponse>> listarPorDono(HttpServletRequest request,
            @RequestParam(defaultValue = "") String usernameDono) {
        List<SalaResponse> lista = this.salaManager.listarPorDono(usernameDono).stream().map(SalaResponse::new)
                .toList();
        return ResponseEntity.ok(lista);
    }
    ...
    public ResponseEntity<SalaResponse> EntrarNaSala(@PathVariable String usernameDono, @PathVariable String nomeSala,
            HttpServletRequest request, @RequestBody @Valid EntrarSalaPorSenha senha) {
    ...
    public ResponseEntity<Void> deletarSala(@PathVariable String usernameDono, @PathVariable String nomeSala,
    ...
    public ResponseEntity<SalaResponse> sairDaSala(@PathVariable String usernameDono, @PathVariable String nomeSala,
            @PathVariable String usernameParticipante, HttpServletRequest request) {
        String username = request.getAttribute("username").toString();
    ...
    @PatchMapping("/{usernameDono}/{nomeSala}/capacidade")
    public ResponseEntity<SalaResponse> alterarCapacidade(
            @PathVariable String usernameDono,
            @PathVariable String nomeSala,
            @RequestBody AlterarCapacidadeRequest body,
            HttpServletRequest request) {
        String username = request.getAttribute("username").toString();
